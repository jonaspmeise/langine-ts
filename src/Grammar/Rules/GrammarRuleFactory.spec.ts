...
describe('Grammar Rule Factory.', () => {
    ...
    describe('build() works.', () => {
        replacementRules.forEach((testCase) => {
            it(`"${testCase.input.getDefinition()}" (${typeof testCase.input}) -> "${testCase.output.getDefinition()}" (${typeof testCase.output}) => Replacement Grammar Rule`, () => {
        ...
        typeRules.forEach((testCase) => {
            it(`"${testCase.input.getDefinition()}" (${typeof testCase.input}) -> "${testCase.output.getDefinition()}" (${typeof testCase.output}) => Type Grammar Rule`, () => {
    ...
    it('In a Replacement Grammar Rule, if the output has References, the input has to have the same References.', () => {
        expect(() => new ReplacementGrammarRule(new MixedSentence('This has a <<Token>>'), new MixedSentence('A <<object>> has this.'))).to.throw(GrammarRuleError);
